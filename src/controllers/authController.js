...
// ثبت Expo Push Token گوشی کاربر (این تابع جدیده)
// فرانت این آدرس رو موقع باز شدن اپ صدا می‌زنه تا بشه بعداً براش پوش فرستاد
exports.savePushToken = async (req, res) => {
    try {
        const { pushToken } = req.body;
        if (!pushToken) return res.status(400).json({ message: 'pushToken الزامی است' });

        // $addToSet یعنی اگه توکن از قبل تو آرایه بود، دوباره اضافه نشه
