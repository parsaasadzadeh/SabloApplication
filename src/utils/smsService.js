...
async function sendInstallmentReminderOneDayBefore(mobile, installmentTitle) {
    const apiKey = process.env.SMSIR_API_KEY;
    const templateId = process.env.SMSIR_INSTALLMENT_1DAY_TEMPLATE_ID;

    if (!apiKey || !templateId) {
        console.warn('⚠️ SMSIR_INSTALLMENT_1DAY_TEMPLATE_ID تنظیم نشده');
        return { success: false, error: 'templateId تنظیم نشده' };
    }
    ...
    try {
        const response = await axios.post(
            SMSIR_BASE_URL,
            {
                ...
                templateId: Number(templateId),
                parameters: [
                    { name: 'TITLE', value: installmentTitle.substring(0, 25) }
                ]
            },
            {
                headers: {
        ...
        const apiError = err.response?.data?.message;
        console.error('SMS.ir 1-day reminder error:', apiError || err.message);
        return { success: false, error: apiError || 'خطا در ارتباط با سرویس پیامک' };
    }
}

async function sendInstallmentReminderTwoDaysBefore(mobile, installmentTitle) {
    const apiKey = process.env.SMSIR_API_KEY;
    const templateId = process.env.SMSIR_INSTALLMENT_2DAYS_TEMPLATE_ID;

    if (!apiKey || !templateId) {
        console.warn('⚠️ SMSIR_INSTALLMENT_2DAYS_TEMPLATE_ID تنظیم نشده');
        return { success: false, error: 'templateId تنظیم نشده' };
    }
    ...
    try {
        ...
        const apiError = err.response?.data?.message;
        console.error('SMS.ir 2-days reminder error:', apiError || err.message);
        return { success: false, error: apiError || 'خطا در ارتباط با سرویس پیامک' };
    }
}
